defmodule Archive.Entry do
  ...
  """
  def load(%__MODULE__{data: data} = e, _) when is_binary(data), do: {:ok, e}

  def load(%__MODULE__{stat: %File.Stat{size: size}} = entry, %Archive{ref: ref}) do
    with {:ok, data} <-
           Nif.safe_call(
  ...
  def load!(%__MODULE__{} = entry, %Archive{} = archive), do: load(entry, archive) |> unwrap!()
  ...
  defimpl Inspect do
    ...
    def inspect(%Archive.Entry{stat: %File.Stat{} = stat} = entry, opts) do
      ...
      size = Archive.format_size(stat.size)
