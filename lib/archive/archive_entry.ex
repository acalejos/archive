defmodule Archive.Entry do
  ...
  """
  use Archive.Nif, as: :entry
  alias Archive.Reader
  alias Archive.Writer
  ...
  def read_data(%__MODULE__{stat: %File.Stat{size: size}} = entry, %Reader{ref: ref}) do
    with {:ok, data} <-
           Nif.safe_call(
  ...
  def read_data!(%__MODULE__{} = entry, %Reader{} = archive),
    do: read_data(entry, archive) |> unwrap!()

  def write_header(
        %__MODULE__{path: path, stat: %File.Stat{} = stat} = entry,
        %Writer{entry_ref: entry_ref, ref: ref}
      ) do
    with :ok <- call(archive_entry_set_pathname(entry_ref, path)),
         :ok <-
           call(archive_entry_copy_stat(entry_ref, Nif.file_stat_to_zig_map(stat))),
         :ok <- call(Nif.archive_write_header(ref, entry_ref)) do
      {:ok, entry}
    end
  end
